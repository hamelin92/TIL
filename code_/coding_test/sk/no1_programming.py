def solution(money, costs):
    answer = 0
    moneys = {1: costs[0], 5: costs[1], 10: costs[2], 50: costs[3], 100: costs[4], 500: costs[5]}
    # 화폐 단위와 코스트를 매칭한 딕셔너리를 만듭니다.
    moneys_list = sorted(moneys.items(), key = lambda item: item[1]/item[0])
    # moneys 딕셔너리를 코스트(value)를 화폐단위(key)로 나눈 값을 기준으로 오름차순 정렬합니다.
    # key와 value가 튜플로 리스트에 저장이 됩니다.
    i = 0
    while money > 0:
        cnt = money // moneys_list[i][0] # 정렬한 리스트에서 가능한 가장 앞의 화폐단위로 목표 금액을 나눕니다.
        #즉 효율이 가장 좋은 화폐를 최대로 생산할 수 있는 개수에 해당합니다.
        answer += cnt * moneys_list[i][1]
        #이를 토대로 answer에 코스트를 누적시키고 money는 화폐단위로 나눈 나머지로 재설정한 후 이 과정을 반복합니다.
        money = money % moneys_list[i][0]
        i += 1
    return answer
