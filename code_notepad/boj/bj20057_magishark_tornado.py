...
print(A)
def tornado_west(arg, a,b):
    if a -2 >= 0:
        arg[a-2][b] = int(arg[a-2][b]+0.02*arg[a][b])
    if a-1>=0:
        arg[a-1][b] = int(arg[a-1][b]+0.07*arg[a][b])
        if b-1>=0:
            arg[a-1][b-1] = int(arg[a-1][b-1]+0.1*arg[a][b])
        if b < N:
            arg[a-1][b+1] = int(arg[a-1][b+1]+0.01*arg[a][b])
    if b -2 >= 0:
        arg[a][b-2] = int(arg[a][b-2]+0.05*arg[a][b])
    if b-1 >= 0:
        arg[a][b-1] = int(arg[a][b-1]+0.75*arg[a][b])
    if a+1 < N:
        arg[a+1][b] = int(arg[a+1][b]+0.07*arg[a][b])
        if b-1>=0:
            arg[a+1][b-1] = int(arg[a+1][b-1]+0.1*arg[a][b])
...
def tornado_east(arg, a,b):
    ...
    if a+2 < N:
        arg[a+2][b] = int(arg[a+2][b]+0.02*arg[a][b])        
    return arg
def tornado_north(arg, a,b):
    if b -2 >= 0:
        arg[a][b-2] = int(arg[a][b-2]+0.02*arg[a][b])
    if b-1>=0:
        arg[a][b-1] = int(arg[a][b-1]+0.07*arg[a][b])
        if a-1>=0:
            arg[a-1][b-1] = int(arg[a-1][b-1]+0.1*arg[a][b])
        if a < N:
            arg[a+1][b-1] = int(arg[a+1][b-1]+0.01*arg[a][b])
    if a -2 >= 0:
        arg[a-2][b] = int(arg[a-2][b]+0.05*arg[a][b])
    if a-1 >= 0:
        arg[a-1][b] = int(arg[a-1][b]+0.75*arg[a][b])
    if b+1 < N:
        arg[a][b+1] = int(arg[a][b+1]+0.07*arg[a][b])
        if a-1>=0:
            arg[a-1][b+1] = int(arg[a-1][b-1]+0.1*arg[a][b])
